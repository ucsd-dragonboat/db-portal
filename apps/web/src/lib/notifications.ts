...
export async function notifyEventPosted(orgId: string, eventIds: string[], excludeUserId: string): Promise<void> {
  ...
  if (!to.length) return;
  const html = renderEmail("New event posted", `<ul>${events.map((e) => `<li>${esc(e.title)} — ${fmtDateTime(e.starts_at)}</li>`).join("")}</ul>`);
...
export async function notifyEventDeadline(event: { id: string; org_id: string; title: string; rsvp_deadline: string | null }): Promise<void> {
  ...
  if (event.rsvp_deadline) {
    ...
    if (to.length) {
      const html = renderEmail("RSVP deadline approaching", `<p>The RSVP deadline for <strong>${esc(event.title)}</strong> is ${fmtDateTime(event.rsvp_deadline)} — about 24 hours from now.</p>`);
...
export async function notifyFormDeadline(form: { id: string; org_id: string; title: string; due_at: string | null }): Promise<void> {
  ...
  if (form.due_at) {
    ...
    if (to.length) {
      const html = renderEmail("Form deadline approaching", `<p>The deadline for <strong>${esc(form.title)}</strong> is ${fmtDateTime(form.due_at)} — about 24 hours from now.</p>`);
...
export async function notifyEventSignup(eventId: string, userId: string, status: Rsvp["status"], ride: Rsvp["ride"]): Promise<void> {
  ...
  const html = renderEmail("Event RSVP", `<p><strong>${esc(who)}</strong> ${label} <strong>${esc(event.title)}</strong>.</p>`);
...
export async function notifyFormSubmitted(orgId: string, formId: string, userId: string, formTitle: string): Promise<void> {
  ...
  if (!to.length) return;
  const html = renderEmail("Form submitted", `<p><strong>${esc(who)}</strong> submitted a response to <strong>${esc(formTitle)}</strong>.</p>`);
...
export async function notifyCarpoolAutoGenerated(orgId: string, eventId: string, result: GenerateResult): Promise<void> {
  ...
  const body = "ok" in result
    ? `<p>Auto-drafted a carpool for <strong>${esc(title)}</strong>: ${result.cars} car${result.cars === 1 ? "" : "s"}, ${result.assigned} assigned, ${result.unassigned} unassigned.</p>`
    : `<p>Couldn't auto-draft a carpool for <strong>${esc(title)}</strong>: ${esc(result.error)}</p>`;
